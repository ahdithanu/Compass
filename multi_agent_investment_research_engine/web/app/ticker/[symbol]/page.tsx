  ...
  return (
    <div className="space-y-6">
      ...
      <Card
        ...
        subtitle={`Top ${evidence.length} signal docs from Chroma backing the thesis`}
      >
        {evidence.length === 0 ? (
          <p className="text-sm text-slate-500">No evidence indexed.</p>
        ) : (
          <ul className="space-y-3 text-sm">
            {evidence.map((e, i) => (
              <li
                key={i}
                className="rounded-md bg-ink-700/60 border border-ink-600 px-3 py-2"
              >
                <div className="flex items-center gap-2 text-xs text-slate-400 mb-1">
                  <span className="font-mono uppercase">{e.signal_type}</span>
