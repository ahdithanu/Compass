...
def ticker_detail(symbol: str) -> dict:
    ...
    # Pull this ticker's price series out of the backtest CSV's shares
    # columns (we don't store raw close per ticker post-run; instead we
    # serialize it from the equity curve plus the trade log on demand).
    bt = _read_backtest()
